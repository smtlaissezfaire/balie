 ...
 */
public class NamedEntityRecognitionNerf extends NamedEntityRecognition {
    ...
    private class UnlistedWordInfo {
        ...
        public NamedEntityType Type() {return m_Type;}
        public int Group() {return m_AliasGroup;}
        private NamedEntityType m_Type;
        private int             m_AliasGroup;
    }
    
    public NamedEntityRecognitionNerf(
    ...
    private void CheckEntityNounAmbiguityNerf() {
        if (m_FullDebug) System.out.println("*************************");
        if (m_FullDebug) System.out.println("* Entity-Noun Ambiguity *");
        // three steps here:
        // 1- identify uppercased, lowercased and uppercased in ambiguous position
        // 2- identify ambiguity (seen upper and lower, upper but always in ambiguous pos, always lowercased).
        ...
        Hashtable<String, ArrayList<Integer>> hLowercased  = new Hashtable<String, ArrayList<Integer>>();   // all indexes of a lowercased form of a word
        Hashtable<String, ArrayList<Integer>> hAmbiguous   = new Hashtable<String, ArrayList<Integer>>();   // all indexes of an ambiguous form of a word
        
        boolean bInsideAllCapitalized = false;

        // 1) loop through tokenlist and mark capitalized, lowercased and ambiguous
        ...
        // 3.1) filter words appearing in both case except if they are part of a capitalized larger expression
        ...
        while (iCur.hasNext()) {
            ...
            if (NERServices.NaturallyLowercased(net)) {
                ...
                if (m_FullDebug) System.out.println("Rejected low&up: " + tok.Raw());
            } else if (NERServices.UncapitalizedMultiWordEntity(tok)) {
                ForceEntityTypeExt(nIdx, new NamedEntityType(NamedEntityTypeEnum.NOTHING, m_TagSetSize, new NamedEntityExplanation(NamedEntityExplanation.ExplanationType.REJECTED_LOWERCASED_AND_UPPERCASED, new Object[]{})));
                if (m_FullDebug) System.out.print("Rejected low&up:");
            }
        }
        ...
        while (iCur.hasNext()) {
            ...
            if (NERServices.NaturallyLowercased(net)) {
                ...
                if (m_FullDebug) System.out.println("Rejected low: " + tok.Raw());
            }
        }
        
        // 3.3) filter words that are always ambiguous except if they are part of a capitalized larger expression
        ...
        while (iCur.hasNext()) {
            ...
            if (NERServices.NaturallyLowercased(net)) {
                ...
                ForceEntityTypeExt(nIdx, new NamedEntityType(NamedEntityTypeEnum.NOTHING, m_TagSetSize, new NamedEntityExplanation(NamedEntityExplanation.ExplanationType.REJECTED_LOWERCASED_OR_AMBIGUOUS, new Object[]{})));
                if (m_FullDebug) System.out.println("Rejected amb: " + tok.Raw());
            } else if (NERServices.UncapitalizedMultiWordEntity(tok)) {
                ForceEntityTypeExt(nIdx, new NamedEntityType(NamedEntityTypeEnum.NOTHING, m_TagSetSize, new NamedEntityExplanation(NamedEntityExplanation.ExplanationType.REJECTED_LOWERCASED_OR_AMBIGUOUS, new Object[]{})));
                if (m_FullDebug) System.out.print("Rejected amb:");
            }
        }
        if (m_FullDebug) System.out.println("*************************");
    ...
    private void PreCheckEntityBoundaryNerf() {
        if (m_FullDebug) System.out.println("************************");
        if (m_FullDebug) System.out.println("* Pre Check Boundaries *");
        ...
        for (int i = 0; i != m_TokenList.Size()-1; ++i) {
            ...
            if (curTok.SentenceNumber() == nextTok.SentenceNumber() && curTok.EntityType().IsEnd()) {
                ...
                // if another entity follows, merge consecutive entities of same "label"
                ...
                if (nextTok.EntityType().IsStart()) {
                    if (m_FullDebug) System.out.print("Adjacent entities: " + curTok.Raw() + " | " + nextTok.Raw());
                    if (curTok.EntityType().GetLabel(m_TokenList.NETagSet()) != null &&
                        ...
                        if (hSharedLabs.size() > 0) {
                            ...
                            if (forceType.Intersect(NamedEntityTypeEnum.Mergable())) {
                                ...
                                if (m_FullDebug) System.out.println(" --> Merged!");
                            } else {
                                if (m_FullDebug) System.out.println(" --> not mergeable");
                            }
                        } else {
                            if (m_FullDebug) System.out.println(" --> not merged");
                        }
                    }
                }
                // a period or a ampersand follows.
                    ...
                    if (secondNextTok.EntityType().IsStart() && curTok.SentenceNumber() == secondNextTok.SentenceNumber()) {
                        if (m_FullDebug) System.out.print("Adjacent entities: " + curTok.Raw() + " | " + nextTok.Raw() + " | " + secondNextTok.Raw());
    ...
    private void CheckEntityEntityAmbiguityNerf() {
        if (m_FullDebug) System.out.println("********************");
        if (m_FullDebug) System.out.println("* Alias Resolution *");
            ...
            if (NamedEntityAliasResolution.ValidAliasToken(curTok)) {
                ...
                if (!neAlias.DominantType().HasNoTag() && !neAlias.DominantType().equals(curType)) {
                    ...
                    if (curTok.EntityType().HasNoTag()) {
                        ...
                        if (m_FullDebug) System.out.println("Force type: " + curTok.Raw() + " --> " + neAlias.DominantType().GetLabel(m_TokenList.NETagSet()));
                    } else {
    ...
    private void ApplyEntityEntityClassifiersNerf() {
        if (m_FullDebug) System.out.println("********************************");
        if (m_FullDebug) System.out.println("* Entity-Entity Classification *");
        ...
        for (int i = 0; i != m_TokenList.Size()-1; ++i) {
            ...
            if (curTok.EntityType().IsStart()) {
                ...
                if (curTypes.TypeCount() > 1) {
                    ...
                    if (m_FullDebug) System.out.print(curTok.Raw()+ " ");
                    ...
                    // also get entity types (if any) of immediate preceding and immediate following thing
                    ...
                    if (m_FullDebug) {
                        for (int j = 0; j != strCtx.length; ++j) {
                            if (strCtx[j] == null) System.out.print("NULL ");
                            else if (!strCtx[j].equals("\n") && !strCtx[j].equals("\r")) System.out.print(strCtx[j] + " ");
                        }
                    }
                    if (m_FullDebug) System.out.print("] ");
                    ...
                    if (netBefore != null && curTypes.Intersect(netBefore)) {
                        ...
                        if (m_FullDebug) System.out.println("--> merged with preceeding of same type");
                        ...
                        if (m_FullDebug) System.out.println("--> merged with following of same type");
                    } else {
                        // classify!
                        ...
                        if (hLabel2NEType.containsKey("month")) {
                            ...
                            if (m_FullDebug) System.out.println("--> classified as: month (special rule)");
                        }
                            ...
                            if (m_FullDebug) System.out.println("--> classified as: " + outcome.Class() + " (" + outcome.Likelihood() + ")");
                            ForceEntityTypeExt(i, new NamedEntityType(hLabel2NEType.get(outcome.Class()), NETagSet.length, new NamedEntityExplanation(NamedEntityExplanation.ExplanationType.CLASSIFIER, new Object[] {curTok.Raw(), hLabel2NEType.get(outcome.Class()).Label(), outcome.Likelihood(), hPriorMap})));
                        } else {
                            // use Round Robin Classification if more than 2
                            // Johannes Fürnkranz  2002 Round Robin Classification Journal of Machine Learning Research 2 (2002), pp. 721-747
                            ...
                            if (m_FullDebug) System.out.println("--> classified as: " + strTypes[nWinner] + " (Probability sum = " + fMaxVote + ")");
                            ForceEntityTypeExt(i, new NamedEntityType(hLabel2NEType.get(strTypes[nWinner]), NETagSet.length, new NamedEntityExplanation(NamedEntityExplanation.ExplanationType.CLASSIFIER_VOTE, new Object[]{curTok.Raw(), hLabel2NEType.get(strTypes[nWinner]).Label(), fMaxVote, fSum, hPriorMap})));
                        }
                    }
                }
            }
    ...
    private void CheckVeryAmbiguousTypes() {
        if (m_FullDebug) System.out.println("******************************");
        if (m_FullDebug) System.out.println("* Check Very Ambiguous Types *");
        ...
        for (int i = 0; i != m_TokenList.Size(); ++i) {
            ...
            if (tok.EntityType().IsStart() && tok.EntityType().Intersect(NamedEntityTypeEnum.HighlyAmbiguous())) {
                ...
                if (CountCapitalizedWords(i) > 1 ||
                    ...
                    if (m_FullDebug) System.out.println("Lot of capitalized: " + tok.Raw());
                    bDisable = false;
                }
                ...
                if (nPreceding > 0 && nFollowing < m_TokenList.Size()) {
                    ...
                    if ((TokenFeature.Feature.HasOpenBracketPunct.Mechanism().GetBooleanValue(precTok.Features()) &&
                        ...
                        if (m_FullDebug) System.out.println("Between brackets: " + tok.Raw());
                        bDisable = false;
                    }
                }
                ...
                if (bDisable) {
                    ...
                    if (m_FullDebug) System.out.println("Rejected very ambigous: " + tok.Raw());
                }
            }
        }
        if (m_FullDebug) System.out.println("******************************");
    ...
    private void PostCheckEntityBoundaryNerf() {
        ...
        if (m_FullDebug) System.out.println("*************************");
        if (m_FullDebug) System.out.println("* Post Check Boundaries *");
        ...
        for (int i = 0; i != m_TokenList.Size()-1; ++i) {
            ...
            if (curTok.SentenceNumber() == nextTok.SentenceNumber() && 
                ...
                if (curTok.EntityType().Intersect(NamedEntityTypeEnum.Mergable()) && !bInsideAllCapitalized && TokenConsts.Is(nextTok.Type(), TokenConsts.TYPE_WORD) && nextTok.EntityType().HasNoTag() && TokenFeature.Feature.StartsWithCapital.Mechanism().GetBooleanValue(nextTok.Features())) {
                    if (m_FullDebug) System.out.println("Entity + capitalized: " + curTok.Raw() + " | " + nextTok.Raw());
                ...
                // a period follows any small word (e.g., middle word)
                    ...
                    if (curTok.SentenceNumber() == secondNextTok.SentenceNumber()) {
                        if (m_FullDebug) System.out.println("Entity + period: " + curTok.Raw() + " | " + nextTok.Raw());
                ...
                // a hyphen or a ampersand follows an organization name.
                    ...
                    // and that is preceded by a mergable type and is followed by a capital letter, but not in a allcap sentence
                    Token secondNextTok = m_TokenList.Get(i+2);
                    if (curTok.EntityType().Intersect(NamedEntityTypeEnum.Mergable()) && 
                       !bInsideAllCapitalized && 
                        ...
                        if (m_FullDebug) System.out.println("Entity + punct: " + curTok.Raw() + " | " + nextTok.Raw());
                        NamedEntityType curType = curTok.EntityType().clone(new NamedEntityExplanation(NamedEntityExplanation.ExplanationType.ENTITY_PUNCT_MERGING, new Object[]{curTok.Raw(), nextTok.Raw()}));
                        curType.RemoveType(NamedEntityTypeEnum.START, null);
                        curType.RemoveType(NamedEntityTypeEnum.END, null);
                        ExtendEntityExt(i, curTok, nextTok, curType);
    ...
    private void CheckUnknownCapitalizedExt() {
        ...
        if (m_FullDebug) System.out.println("*****************************");
        if (m_FullDebug) System.out.println("* Check Unknown Capitalized *");
        ...
        while (eCur.hasMoreElements()) {
            ...
            // annotate every capitalized occurences of this word according to this type
            ...
            if (alIndexes != null) {
                ...
                while (iCur.hasNext()) {
                    ...
                    if (curTok.EntityType().HasNoTag()) {
                        ...
                        if (m_FullDebug) System.out.println("Unknown capitalized tagged: " + curTok.Raw() + "(" + curTok.EntityType().GetLabel(m_TokenList.NETagSet()) + ")");
                    }
                }
            }
        }
    ...
    private int CountCapitalizedWords(int pi_Start) {
        ...
        if (TokenFeature.Feature.StartsWithCapital.Mechanism().GetBooleanValue(curTok.Features())) nNumCap++;
        while (!curTok.EntityType().IsEnd()) {
            ++pi_Start;
    ...
    private Hashtable<String,ArrayList<Integer>>            m_CapWord2Index;         // all indexes of capitalized form of a word 
    private Hashtable<String, UnlistedWordInfo>              m_UnlistedWords;         // lookup of words that are part of entities but are not lexicon entries
